...
type THeadType<A extends any[]> = A extends [infer H]
  ? H
  : A extends [infer H, ...any[]]
  ? H
  : never;

type TLastType<A extends any[]> = A extends [infer L]
  ? L
  : A extends [...any[], infer L]
  ? L
  : never;

type TTailTypes<A extends any[]> = A extends [any]
  ? []
...
> = TLength<I> extends N
  ? T
  : TDropHead<N, TTailTypes<T>, TPrepend<THeadType<T>, I>>;
